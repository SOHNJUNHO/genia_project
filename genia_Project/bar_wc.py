...
## 수동으로 주제에 따라 분류....
df_company = df_freq.loc[['학원','밀크티','홈런','엘리하이','윙크','스마트올','씽크빅','빨간펜','이비에스','구몬','공부방','엠베스트','엄마표','온리원','눈높이','아이캔두','하프스터디','연필영어','와플','밀크티아이','어학원','윤','리딩게이트','쿠키','도요새','천재교육','비상'],:]
df_company.loc['온리원','frequency']=df_company.loc['온리원','frequency']+df_freq.loc['와이즈','frequency']+df_freq.loc['수박씨','frequency']	 ## 수박씨,와이즈캠프가 온리원으로 이름 변경
...
df_company.sort_values(by='frequency', inplace=True, ascending=False)
...
df_subject.sort_values(by='frequency', inplace=True, ascending=False)


df_paper=df_freq.loc[['최상위','개념','심화','응용','사고력','기본','원리','만점왕','디딤돌','실력','우등생','팩토','우공비','큐브','1031','혼공','기탄'],:]
df_paper.loc['우등생','frequency']=df_paper.loc['우등생','frequency']+df_freq.loc['해법','frequency']
df_paper.sort_values(by='frequency', inplace=True, ascending=False)


df_contents=df_freq.loc[['연산','파닉스','리딩','논술','독해','게임','개뼈','뼈','오투','문법','도서관','이야기','만화','실험','특강','동영상','원서','도서','원어민','발음','스피킹','놀이','북클럽','회화'],:]
df_contents.sort_values(by='frequency', inplace=True, ascending=False)


df_interest=df_freq.loc[['습관','병행','수업','문제집','책','선생','학습지','문제','화상','강의','학교','교재','인강','컨텐츠','지면','교과서','영상','교과연계','연계','유튜브','교과','관리'],:]
...
df_interest.sort_values(by='frequency', inplace=True, ascending=False)
...
for df,title in zip(df_list,titles):
    ax = df.plot.bar(figsize=(25,13),legend=None)
